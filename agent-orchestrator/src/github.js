...
export function validateChunkCount(chunks, maxChunks) {
  validatePositiveInteger(maxChunks, "MAX_DIFF_CHUNKS");
  if (!Array.isArray(chunks)) throw new Error("chunks must be an array");
  if (chunks.length > maxChunks) {
    const error = new Error("Diff requires more than MAX_DIFF_CHUNKS review chunks");
    error.code = "DIFF_TOO_MANY_CHUNKS";
    error.chunkCount = chunks.length;
    error.limit = maxChunks;
    throw error;
  }
  return chunks;
}
...
export function validateGitHubConfig({ repo = "", base = "", head = "" }) {
  ...
  if (present !== 0 && present !== 3) throw new Error("GITHUB_REPO, GITHUB_BASE, and GITHUB_HEAD must be set together");
  return { configured: present === 3, repo: values[0], base: values[1], head: values[2] };
}
...
function splitOversizedText(text, maxChars) {
  ...
  for (const line of text.split(/(?<=\n)/)) {
    ...
    if (current.length + line.length > maxChars && current) { chunks.push(current); current = ""; }
    current += line;
  }
...
export function splitDiffIntoChunks(diff, maxChars) {
  ...
  for (const section of sections) {
    ...
    if (current.length + section.length > maxChars && current) { chunks.push(current); current = ""; }
    current += section;
  }
  if (current) chunks.push(current);
  return chunks;
...
async function fetchDiffUrl({ url, token, timeoutMs, maxBytes }) {
  ...
  try {
    const headers = { "Accept": "application/vnd.github.v3.diff", "User-Agent": "fishcrm-agent-orchestrator" };
    if (token) headers.Authorization = "Bearer " + token;
    const response = await fetch(url, { headers, signal: controller.signal });
    if (!response.ok) throw new Error("GitHub diff error " + response.status + ": " + await response.text());
...
export async function fetchGitHubDiff({ repo, base, head, token, timeoutMs = 120000, maxBytes = 250000 }) {
...
export async function fetchGitHubPullRequestDiff({ repo, prNumber, token, timeoutMs = 120000, maxBytes = 250000 }) {
  if (!repo) throw new Error("repo is required, e.g. mangostik/First-");
