...
async function requestClaude({ apiKey, model, prompt, maxOutputTokens, timeoutMs, signal }) {
  return requestWithTimeout({
    provider: "Claude", timeoutMs, signal,
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      try {
        ...
        const recoveryPrompt = [prompt, "", "FORMAT RECOVERY: Return one JSON object only. Do not use prose, Markdown, code fences, or explanations outside that object."].join("\n");
